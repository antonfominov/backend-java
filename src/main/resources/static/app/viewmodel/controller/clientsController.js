Ext.define('AppExtJS.viewmodel.controller.clientsController',
		{
			extend : 'Ext.app.ViewController',
			alias : 'controller.viewportClientsController',

			onAddClick : function() {
				Ext.create('AppExtJS.viewmodel.view.ClientsEditor', {}).show();
			},
			...
			clientCreate : function() {
				...
				Ext.getCmp('clientsEditor').destroy();
				Ext.data.StoreManager.lookup('clients').reload();
			},

			onRemoveClick : function(btn) {
				if (Ext.getCmp('clients').getView().getSelectionModel()
						.getSelection()[0] != undefined) {
					Ext.MessageBox.show({
						title : 'Внимание',
						msg : 'Удалить выбранного клиента?',
			...
			onRefreshClick : function() {
				Ext.data.StoreManager.lookup('clients').reload();
				Ext.getCmp('clients-combo').clearValue();
			},
			updateClick(){
			...
			myItemClick : function() {
				Ext.getCmp('clients').setReadOnly(false);
			},
